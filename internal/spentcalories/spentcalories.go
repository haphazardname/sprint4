...
func distance(steps int, height float64) float64 {
	return (float64(stepLengthCoefficient) * height) * float64(steps) / mInKm
}
...
func TrainingInfo(data string, weight, height float64) (string, error) {
	...
	return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f", typeTrain, trainDur.Hours(), distance(steps, height), meanSpeed(steps, height, trainDur), calories), nil
}
...
func WalkingSpentCalories(steps int, weight, height float64, duration time.Duration) (float64, error) {
	...
	meanSpeed := meanSpeed(steps, height, duration)
	durationInMinutes := duration.Minutes()
	return (height * meanSpeed * durationInMinutes / minInH) * walkingCaloriesCoefficient, nil
